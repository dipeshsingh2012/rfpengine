...
export const KBAddSourceModal: React.FC<KBAddSourceModalProps> = (props) => {
  ...
  return (
    <div className="modal-backdrop" onClick={props.onClose}>
      <div className="modal-card source-config-modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h3>Add Automated Grounding Connector</h3>
          <button className="icon-button" onClick={props.onClose}><X size={16} /></button>
        </div>
        <form onSubmit={props.onSubmit}>
          <div className="modal-body form-grid">
            ...
            {props.sourceType === "github_docs" && (
              <>
                <div className="form-group"><label>Repository (owner/repo)</label><input value={props.repo} onChange={(e) => props.setRepo(e.target.value)} required /></div>
                <div className="form-group"><label>Branch</label><input value={props.branch} onChange={(e) => props.setBranch(e.target.value)} required /></div>
                <div className="form-group"><label>File Patterns</label><input value={props.files} onChange={(e) => props.setFiles(e.target.value)} required /></div>
              </>
            )}
            {props.sourceType === "cloud_storage" && (
              <div className="form-group"><label>Storage Folder / S3 Bucket</label><input value={props.folder} onChange={(e) => props.setFolder(e.target.value)} required /></div>
            )}
            <div className="form-group">
