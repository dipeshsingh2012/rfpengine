...
1. **Lack of a single canonical source of truth**: Search indexes (Algolia/Pinecone) are optimized for indexing and retrieval, not for ACID transactions, referential integrity, relational modeling, or audit history.
...
2. **Algolia & Pinecone**: Manage knowledge base document chunks:
   - High-volume document chunks (300–500 tokens) are indexed directly into **Algolia** (sparse keyword search and full text storage) and **Pinecone** (dense vector search).
...
- **Fast Dual-Retrieval**: Algolia and Pinecone handle search and direct document hydration with zero extra relational round-trips.
...
### Negative / Trade-offs
- Re-indexing entire knowledge bases requires re-uploading source files or re-syncing to Algolia and Pinecone.
